...
CMV::CMV(int id):TimeRange(id)
{
}

void CMV::CheckConfig(const configuration::Json &conf)
{
	...
	try
	{
		for(auto device_id : conf.GetArray("ht_device_ids"))
			Devices::GetByID<Weather>(device_id); // Check device exists
	}
	catch(exception &e)
	{
		throw invalid_argument("Associated hygrometer is mandatory");
...
void CMV::reload(const configuration::Json &config)
{
...
void CMV::check_timeranges(const configuration::Json &conf, const string &name)
{
	if(!conf.Has(name))
		return;
...
en_wanted_state CMV::get_wanted_state(configuration::Json *data_ptr) const
{
	...
	try
	{
		...
		{
			auto ht = Devices::GetByID<Weather>(ht_device_id);
...
bool CMV::Depends(int device_id) const
{
